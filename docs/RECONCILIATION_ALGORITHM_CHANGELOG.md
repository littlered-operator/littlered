...
- **Why that is a merge and not a failed command** (verified against Redis 8.4.2 `src/cluster_legacy.c`, Redis 7.2 `src/cluster.c` and Valkey 8.1 — all three agree):
    1. **MEET has no membership validation at all.** `clusterStartHandshake` (`cluster_legacy.c` ~:1990-2047) validates `inet_pton` and the port range. Nothing checks the target's cluster identity, epoch, slots or size — no such field exists in the protocol.
    2. **The receiver trusts an inbound MEET.** ~:2937-2962: a MEET from an unknown sender creates the node in handshake and ingests its whole gossip section, with the upstream comment *"we still process the gossip section here since we have to trust the sender because of the message type."*
    3. **The initiator adopts the responder's identity.** ~:2977-2999: on a handshake link, if the responder's node ID is unknown, `clusterRenameNode(link->node, hdr->sender)` runs and the handshake flag is cleared — the foreign node becomes a full member. Bidirectional, and transitive via gossip.
    4. **Node-ID keying protects only ESTABLISHED nodes** — and does so exactly as one would hope: ~:3004-3020, a PONG whose sender ID mismatches the recorded one gets `CLUSTER_NODE_NOADDR` and its link freed. Upstream states the warm-IP reasoning verbatim in the *gossip-learn* path (~:2214-2218): *"we cannot simply start a handshake against this IP/PORT pairs, since IP/PORT can be reused already, otherwise we risk joining another cluster"* — which is why a gossip-learned node is created with the node ID **from the payload**, never via handshake. **MEET is the one operation that creates a fresh identity binding, which is why it is the hole.**
    5. **Authentication does not close it.** `grep -n 'requirepass\|masterauth' src/cluster_legacy.c` returns **zero** hits in all three versions: the cluster bus is unauthenticated, and the merge travels on the bus. So this is not a documentation-of-`spec.auth` matter (contrast LR-039, where auth *is* the remaining mitigation for Sentinel's address-adoption path).
...
- **Fix, part 2 (defence in depth) — attribute the address before creating an identity binding.** New pure seam in `internal/redis/cluster_state.go`: `AttributeMeetTarget(MeetCandidate, ourNodeIDs, shards) MeetVerdict`, with the Step 1 adapter `(*ClusterGroundTruth).PlanPartitionMeets(shards)`. The safety property is **"never MEET an address this instance has not attributed to itself this pass"**, and a target is admitted only when it was **identified this pass** (an identity probe answered *and* its own `CLUSTER NODES` view was gathered) and one of:
    ...
    - **fresh** — an isolated (single-entry) node table owning no slots: a new, restarted or wiped pod of ours. This is bootstrap's normal case.
    - **survivor** — isolated, but owning **exactly** the slot range this instance assigns to that pod name (`GenerateSlotRanges` × `ShardIndexFromPodName`).
  Everything else is skipped with a named verdict (`unidentified` / `no-gossip-view` / `unattributed`) on the audit log — a silently suppressed MEET is exactly what makes a future partition-healing bug hard to diagnose. **Zero extra Redis round-trips in the repair loop**: the gossip view is `gt.KnownNodes`, already retained for partition detection (LR-014), and the slots are already gathered.
...
- **Cross-mode / parity audit (CLAUDE.md §7 rule 11).** Every site that creates an identity binding or acts on a cache-backed pod IP was audited:
    ...
    | migration MEET **seed** | **deliberately not gated** | a legacy `{name}-cluster-N` pod of an already self-consistent cluster, identified this pass and behind the `LegacyShapePreserved` facts. The survivor clause is keyed on per-shard pod names, so applying the predicate would refuse a legitimate single-node legacy cluster (`shards:1`, `replicasPerShard:0`) and the migration would never start — the LR-018/LR-023 "repair step that can never fire" trap |
