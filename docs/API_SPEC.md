...
status:
  ...
  staleMasterNameForeignSince: "2026-08-26T10:00:00Z" # Set while a leftover master name points somewhere unattributable; settles the Foreign reading
  sentinels:
    ready: 3
    total: 3
...
| `staleMasterNameForeignSince` | `Time` | Set when the operator first observes a **stale** Sentinel master-name entry pointing at an address that is neither one of this instance's pods nor flagged down; cleared as soon as that reading stops. It exists only to settle the `StaleMasterName` condition's `Foreign` reading for 30s, because during an ordinary rename a just-replaced pod's address is already out of the pod list while Sentinel has not yet flagged it down (`down-after-milliseconds`) — indistinguishable from a captor's live master. Nothing is pruned in that state either way, so this is an observability timer, like `forsakenSince` (sentinel mode). |
