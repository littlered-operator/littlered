...
| `updateStrategy.minReadySeconds` | `int32` | No | Mode-dependent | Minimum seconds a pod must be ready before the next pod is restarted. **Cluster mode with replicas**: defaults to 30s. **Sentinel mode**: defaults to 35s for sentinel-managed failover. **Standalone/0-replica**: defaults to 0s. Range: 0-300. |

> **Cluster mode: `minReadySeconds` is not the safety mechanism.** An operator-triggered rolling update of a cluster shard is gated on *state*, not on a timer: the operator holds the shard's StatefulSet at a `rollingUpdate.partition` and lowers it one ordinal at a time, only once every pod at or above it is at the StatefulSet's `UpdateRevision`, Ready per the kubelet, **and** a link-`up` replica of that shard's slot owner (ADR-017). `minReadySeconds` is retained as defence in depth on top of that gate. It still governs sentinel, failover and standalone rollouts, and — like the partition gate — it does not govern a manual `kubectl rollout restart`, a node drain or an eviction.
...
status:
  ...
  # Declared heavy operations (ADR-020) — all modes; sentinel mode is the only
  # one with a registered operation today
  acknowledgedOperations:       # Per operation: the value fingerprint that FINISHED
    - name: SentinelMasterNameRename
      fingerprint: 3f9c1a7e08b2d456          # HMAC-SHA256(instance UID, name|value), truncated
      acknowledgedAt: "2026-08-29T10:04:11Z"
  operation:                    # The one running now, plus its queue — monitoring only
    name: SentinelMasterNameRename
    startedAt: "2026-08-29T10:01:02Z"
    reason: Running             # Running | Blocked | Stalled
    pending: []                 # Queued behind it, in the order they will run
  ...
  quarantinedSince: "2026-08-22T09:01:00Z" # Set while a forsaken instance is held at 0 replicas so the capturing instance can heal
  quarantineAttempts: 1                    # How many times this instance has been quarantined as forsaken (bounded, then latched)
  sentinels:
    ready: 3
    total: 3
...
| `quarantinedSince` | `Time` | Set when the operator quarantines a forsaken instance — its Redis **and** Sentinel StatefulSets are held at 0 replicas so the *capturing* instance can prune the departed pods from its own Sentinel topology through its existing ghost-replica pruning. Also the only thing that *holds* the quarantine: with no pods there is no reachable monitoring Sentinel, so the capture signature itself disappears. Cleared when the settling period (120s) elapses and the pods are allowed back — they return empty, which the existing no-data leaderless reseed handles (sentinel mode). It is also what the operator reads *before* it applies either StatefulSet, so a quarantine holds without any pass putting the pods back; consequently, releasing a **latched** instance by hand means clearing this field (and `quarantineAttempts`), not editing the StatefulSets, which are server-side-applied from this decision every pass. |
| `quarantineAttempts` | `int32` | How many times this instance has been quarantined as forsaken. `2` means it was captured again after its first quarantine and reseed. Bounded: at the limit (2, or **1** when the instance's own configuration is known-dangerous — auth disabled *and* the effective master name is the shared legacy `mymaster`) the instance stays at 0 replicas instead of being released again, because every recapture also re-pollutes a healthy neighbour. Reset only when the instance reaches a healthy `Running` state, never merely because the capture signature stopped being observable (sentinel mode). |
| `observedGeneration` | `int64` | Last processed `.metadata.generation` |
| `acknowledgedOperations` | `[]OperationAck` | One row per registered **heavy operation** (ADR-020) recording the value fingerprint the operator has **finished** carrying out. A candidate whose freshly computed fingerprint differs from its row here is unfinished work from a spec change; equal means converged; a candidate with **no row** on an already-initialized instance is *seeded* — the ack is written without anything being run, which is what stops an operator upgrade declaring an operation for every instance in a fleet. This is the one thing the mechanism persists, and it is consistent with ADR-006 rather than an exception to it: ADR-006 forbids persisting what is **recomputable**, and "this work finished at T" is an observation of an event that cannot be re-derived from live state — the same ground `leaderlessSince`, `ghostMasterStuckSince`, `forsakenSince`, `quarantinedSince` and `quarantineAttempts` already stand on. Bounded by the registry, not by time: one row per operation *name*, updated in place, with no expiry (a missing row is re-seeded, never re-run). |
| `acknowledgedOperations[].name` | `string` | Registry key of the heavy operation, e.g. `SentinelMasterNameRename` |
| `acknowledgedOperations[].fingerprint` | `string` | Identifies **which value** completed: `HMAC-SHA256(instance UID, name‖value)`, hex, truncated to 16 chars. Deliberately not the value — status is readable by anyone who can `get` the CR, and the fingerprinted value is a password once auth joins the registry, so a bare digest of a short secret would be a dictionary lookup. It also cannot be reversed into "the previous master name", which is how ADR-018's refusal to remember one is structurally enforced. Naming an intent by its *field* instead would match the wrong edit, since the same field changes repeatedly. |
| `acknowledgedOperations[].acknowledgedAt` | `Time` | When the operator observed the operation complete — or, for a seeded row, when it first recorded that the instance was already in the state its spec asks for |
| `operation` | `OperationStatus` | The heavy operation in progress and the queue behind it. **Monitoring surface only**: re-derived every pass from (spec, `acknowledgedOperations`, live StatefulSets) and never read back by any decision; losing it costs nothing. Absent when no operation is running. |
| `operation.name` | `string` | Registry key of the running operation |
| `operation.startedAt` | `Time` | When the operator first ran it. Anchors the operation's `StallAfter` budget, which raises a condition and an event and then waits — there is deliberately no auto-exit timer (ADR-017: a timer is the defect with a delay) |
| `operation.reason` | `string` | `Running`, `Blocked` (the driver reported it cannot proceed — never auto-skipped) or `Stalled` (past `StallAfter`) |
| `operation.pending` | `[]string` | Operations queued behind this one, in the order they will run. Heavy operations serialize, so a wedged operation holds the queue indefinitely and loudly rather than being skipped. There is no precedence field and none is coming (ADR-020 Alternatives E and F): ordering comes from the operation already running, from admission refusing two simultaneous heavy spec edits, and from declared `Requires` dependencies on the registry entry |
...
| `Forsaken` | ✅ | Sentinel mode only: `True` means this instance has been CAPTURED by another Sentinel deployment sharing its master name — every reachable Sentinel serves a live master that is not one of its pods, and no pod of its own is a master. Terminal by design: recovery is declined (ADR-015 §9.2), so the operator stops managing the instance and re-examines it at the steady interval; it stays `Ready=False` until a human runs the capture-recovery runbook in `docs/USAGE.md`. `False` records that its Sentinels are serving its own topology. The reason distinguishes the operator's response: `Captured` (verdict only), `Quarantined` (held at 0 replicas, message names the attempt out of the limit), `QuarantineLatched` (attempt budget spent — a configuration problem, not something to retry), `QuarantineRefusedDataPresent` / `QuarantineRefusedDataUnknown` (forsaken, but not quarantined because a pod holds data the captor does not have, or could not be proven empty — the operator could not dial it *and* the kubelet still reports its redis container Ready; a not-Ready redis holds no data in a pure in-memory instance and does not block) |
| `StaleMasterName` | ✅ | Sentinel mode only: `True` means this instance's Sentinels monitor at least one master name **other** than `spec.sentinel.masterName`. That is what editing the field used to leave behind permanently — Sentinel persists every `sentinel monitor <name> …` it is given and nothing removed the old one — so one master was monitored under two names by two independent failover state machines. `False`/`Converged` is the steady state: every reachable Sentinel monitors exactly the desired name and nothing else. Reasons: `Pruning` (`True` — stale names observed and being removed this pass; the message names them, and any Sentinel skipped because it does not carry the desired name yet, which is Rule 0's job and is expected to clear on a later pass), `Deferred` (`True` — observed, but a gate refuses; the message names **which** gate, e.g. no living master of ours, a failover in flight under some monitored name, or a Sentinel quorum that is not reachable), and `Foreign` (`True` — a stale entry points at an address that is neither one of this instance's pods nor flagged down, **on an instance whose own Redis StatefulSet is settled**, so something else is alive there; one Warning event per transition: **do not rename to escape a capture**, let the quarantine complete first and rename the empty instance). While the Redis StatefulSet is *not* settled the operator withholds attribution entirely (LR-050) and reports that same reading as `Deferred` naming the rollout gate — a pod the rename has just replaced looks exactly like a captor's master for a whole `down-after-milliseconds`. Nothing is ever pruned under `Deferred` or `Foreign`. The condition is a transient progress/diagnostic surface, not a terminal verdict, and it never affects `Ready` |
...
| `ClusterRolloutBlocked` | ✅ | Cluster mode only: `True` means a shard's rolling update is **held** because the shard is not redundant — a replaced pod is at the StatefulSet's `UpdateRevision` and Ready per the kubelet, but has no attachment at all to its shard's slot owner, and has been in that state for longer than the operator's 120s reattach budget. While it holds, the shard's remaining pods (including its master) are not taken down, so the instance keeps serving and its data is intact; the update simply does not finish, and there is deliberately no timer that releases it. Manual release is raising that StatefulSet's `spec.updateStrategy.rollingUpdate.partition` by hand — the condition message spells out the `kubectl patch`. Reasons: `ShardNotRedundant` (`True`), `RolloutProgressing` (`False`, recorded only once the condition has existed — a cluster that has never been blocked carries no such condition). A Warning event `ClusterRolloutBlocked` is emitted **once per transition**, not per reconcile. The condition is **advisory**: it reports, and never changes which partition the operator applies — the emitted partition is identical whether a hold is reported as blocked or not. It is never set for a pod that IS attached to the owner but whose replication link is still down: that is a full sync in flight, which is dataset-dependent, genuinely unbounded and real progress. A dedicated condition rather than `Ready=False` by design (ADR-017) — a stalled rollout is a rollout that has not finished, not an unhealthy instance. `Ready` will nonetheless read false for the ordinary reason that a not-yet-reattached replacement is an empty master; this condition is what distinguishes "converging" from "stuck" |
| `OperatorCannotAuthenticate` | ✅ | All modes: `True` means at least one of this instance's own pods **refused the operator's credential** — `NOAUTH`, `WRONGPASS`, `ERR Client sent AUTH, but no password is set`, or `ERR invalid password`. Such a pod is not unreachable in any useful sense: something answered on that address and spoke the protocol well enough to turn the operator away, so it is a **live server whose keyspace cannot be read**. Before this condition existed the state was invisible — the probe error was discarded and every field the gather would have filled stayed at its zero value, so a credential mismatch was byte-identical to a dial timeout and a fleet holding its entire dataset read as holding nothing (LR-051). The message names the pods **and what each distinct server said**, because the reply is the diagnosis: `WRONGPASS` means the password in the Secret was changed under a running instance (a Secret edit rolls no pods, so they keep enforcing the previous password indefinitely), while `NOAUTH` means the operator lost a password the pods still enforce. Reasons: `CredentialRejected` (`True`), `CredentialAccepted` (`False`, recorded only once the condition has existed — an instance that has never had a credential problem carries no such condition). One Warning event per **transition**, not per reconcile. **While it is `True` the operator refuses every action that would discard data** — Rule L's leaderless reseed, the ghost-master election, `planFailover`'s seed/promote, and the ADR-016 capture quarantine — reporting `RefusedDataUnverified` on `LeaderlessRecovery` / `GhostMasterRecovery` / `FailoverRecovery` and `QuarantineRefusedDataUnknown` on `Forsaken`. That refusal is deliberately **not** overridable by `allowUnsafeRebootstrapOnDeadlock`: that opt-in authorizes discarding a set of holders the owner could see, and here the operator cannot see the set at all — the remedy is to fix the credential, which destroys nothing. In cluster mode the condition is reported but gates nothing, because the one cluster decision that deletes pods already takes its evidence from the kubelet rather than from the operator's dial. It never affects `Ready` |
| `OperationInProgress` | ✅ | All modes (only sentinel mode has a registered operation today): `True` means a declared **heavy operation** is being carried out — a human edited one of the small, registered set of spec fields whose change cannot safely proceed alongside regular healing, so the operator has taken the early branch that suppresses that healing until the work completes (ADR-020). `True` is a normal, expected state, not a fault; it is reported loudly because the instance is deliberately not being healed while it holds, and an operation that never completes is the failure mode the mechanism exists to make visible. Reasons mirror `status.operation.reason`: `Running`, `Blocked` (the driver reported it cannot proceed — never auto-skipped) and `Stalled` (past the operation's `StallAfter` budget). There is deliberately **no auto-exit timer**, so a stalled or blocked operation stays `True` until a human acts. It never affects `Ready`: an instance mid-rename is not an unhealthy instance |
