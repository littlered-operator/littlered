...
- **Baseline — classic pre-8.4 dance** (compatibility path; what `redis-cli --cluster reshard` does). **This is the design baseline** because many deployments run pre-8.4 Redis or engines/editions without native ASM (see §7.4). Per slot range moved S→D, per slot:
  ...
  **Implemented incrementally** (`reshardViaDance`): each reconcile (idempotently) re-marks the range IMPORTING/MIGRATING, drains up to **`ReshardMaxKeysPerReconcile`** keys (default 2000) in **`ReshardKeyBatchSize`**-key `MIGRATE` batches (default 128), and **flips ownership (`SETSLOT NODE`) only once the whole range is drained** — broadcast to every reachable master. Because ownership flips only at the end, the source keeps owning the range in gossip throughout, so `PlanReshard` keeps emitting the same move and the executor **resumes from the cluster's own slot markers** (no persisted cursor). The per-reconcile bound is a **key count, not wall-clock** — deterministic, unit-testable, and it keeps each pass short so it never hogs the single reconcile worker. A single `MIGRATE` call is bounded by **`ReshardMigrateTimeoutMillis`** (default 5000) — the real anti-hang guard; "can't move a batch within the budget" simply means the range spans more reconciles, never an error. A key too large to move within that timeout stalls the reshard and is logged **loudly** (never a silent no-op). New client methods: `ClusterSetSlots{Importing,Migrating,Node}` and `ClusterCountKeysInSlots` (pipelined — one round-trip to mark/flip/count a whole range), plus `ClusterGetKeysInSlot`/`MigrateKeys`. `B`, the per-reconcile budget, and the MIGRATE timeout are advanced CR fields (`spec.cluster.reshard*`).
...
Both paths are **asynchronous from the operator's view** and run **incrementally across reconciles** — kick off (dance batch or `IMPORT`), then on subsequent fast requeues observe progress via slot-ownership in the normal gather (and, for ASM, `STATUS` for the in-flight/re-entrancy check) rather than blocking a reconcile, consistent with the non-blocking idempotent loop (CLAUDE.md rule 1, pillar 3.5). Re-entrant safety needs **no persisted operator state**: for the dance, the source still owns the whole range until the final flip, so `PlanReshard` re-emits the same move and the executor resumes from the IMPORTING/MIGRATING markers already on the nodes; for ASM, `ClusterMigrationInFlight` (a `STATUS` check on the destination) prevents relaunching `IMPORT` while a task runs.
...
## 11b. Validation — pre-8.4 dance (s1 lab, 2026-07-29)

Verified end-to-end on a live **Redis 7.4.0** cluster (no `cluster_slot_migration_*` → `AtomicSlotMigration=false` → dance path), with a deliberately large moved range to force multi-pass draining:

- **Setup:** 5000 keys concentrated in one shard-2 slot (hash tag), consolidated onto cluster-0 via `redis-cli --cluster reshard`, cluster-2 turned into a fresh empty master.
- **Result:** the operator drained the range in **three passes** (`movedThisPass` 2048 → 2048 → 904 = 5000, each a separate reconcile, respecting `ReshardMaxKeysPerReconcile=2000`), **resuming from the on-node IMPORTING/MIGRATING markers** with no operator state, then logged `drain complete, ownership flipped to dest`. End state: `cluster_size:3`, phase `Running`, cluster-2 owns 10923-16383 with **all 5000 keys** (`COUNTKEYSINSLOT=5000`), spot-checks correct, and **zero leftover `[...]` markers**. ~7 s across 4 reconciles.

**Bug found and fixed by this validation (`ParseClusterNodes`, commit `1a9733d`).** The first dance run *wedged after one drain pass*: `lrctl verify` showed the source's `slots:` field polluted with the per-slot migrating `[slot->-id]` / importing `[slot-<-id]` notations. The `CLUSTER NODES` parser took every trailing field as an owned slot (`parts[8:]`), a **latent bug** — nothing in the operator ever marked slots IMPORTING/MIGRATING before the dance. Effects: the source's `Slots` got bracket tokens (`PlanReshard` → "unparseable → defer"), and the *importing but slotless* destination had `len(Slots)>0` → counted as a slot-owning master → `CountMasters==shards` and `HasEmptyMasters==false` → `IsHealthy` wrongly returned true → the operator declared the cluster healthy and abandoned the reshard, stranding keys on a non-owner. Fix: skip tokens starting with `[`. Guarded by `TestParseClusterNodes_SkipsMigrationMarkers` (red-first). This is exactly the class of latent defect the e2e exists to catch — it is invisible to unit tests that never exercise a mid-migration topology.

**Residual (ASM STATUS parse):** the `CLUSTER MIGRATION STATUS` non-empty/in-flight parse still lacks a live exercise (ASM migrations complete before an in-flight poll; the dance path does not use `STATUS` at all). Low risk — the documented reply shape is unit-tested — but worth a targeted check if a large ASM migration is ever observed in the field.
...
2. ~~**Batch granularity**~~ **RESOLVED (§7.2):** bound per reconcile by a **key count** (`ReshardMaxKeysPerReconcile`, default 2000), not wall-clock — deterministic, unit-testable, keeps each pass short for the single reconcile worker. `MIGRATE` batch = `ReshardKeyBatchSize` (128). The real anti-hang bound is the per-`MIGRATE` `ReshardMigrateTimeoutMillis` (5000). All three are advanced CR fields.
