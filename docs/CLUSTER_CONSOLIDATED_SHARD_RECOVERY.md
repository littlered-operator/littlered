...
**Pre-8.4 dance:** implemented (`reshardViaDance`, incremental, key-count budget) and unit/lint/envtest green; **end-to-end validation on a pre-8.4 engine is pending** (deploy an older-tag cluster on the lab so `AtomicSlotMigration=false`). This is where the `STATUS` non-empty parse and the slow-migration resume path get their real exercise.
