...
{{/*
Manager (reconcile) RBAC rules — `littlered.managerRules` — are defined in the
GENERATED helper templates/_managerrules.tpl (written by `make manifests` from
config/rbac/role.yaml). Shared VERBATIM between the cluster-scoped ClusterRole
(default / deny-list modes) and the per-namespace Roles (allow-list mode), so
scoping never silently drops a permission.
*/}}
