...
# A Helm chart cannot reference files outside its own directory, and CRDs must
# live at charts/littlered/crds/, so the generated CRDs and RBAC rules are
# *copied* into the chart. Both copies are checked in; `make verify-generated`
# fails if either drifts from config/. Neither copy step may fail silently --
# a swallowed error ships a chart with stale CRDs while reporting success.
CHART_CRD_DIR = charts/littlered/crds
