...
**Fix (LR-017):** Made `GatherClusterState` probe concurrently; renamed `ClusterProbeTimeout` → mode-neutral `ProbeTimeout` (3s) and wrapped the gatherer's `GetRedisState` and every `SentinelClient` read-path address loop in `context.WithTimeout(ctx, ProbeTimeout)`, so a dead address fails in ≤3s regardless of retries. The sentinel-mode completion of LR-012; the worked example for the cross-mode-parity rule (§7). See changelog LR-017. **Cloud re-verification pending.**
