...
- **Start-gated kill-9 yield** (the ADR-001 same-IP hazard, re-owned; corrected by LR-038): before `exec` the startup script records the epoch it is **starting under** in an EmptyDir marker (`/data/littlered-started-under-epoch`). The marker survives a container restart (same pod, same IP, wiped dataset) but not a pod replacement, so its presence means exactly "I am a restarted process holding no data", and the pod **parks in the wait-loop — parking IS the yield**. **Two rules, because the roles carry different risk:** a *replica* assignment is honored on epoch ordering alone (starting as a replica of the live master loses nothing); a *master* assignment additionally requires `master-start-authorized-epoch`, which the operator stamps only **after** observing the restart and establishing that no data is at risk (pure `masterStartAuthorizedFor` — seed and bootstrap only, **never** a promotion). **Why ordering alone is not enough:** an in-place promotion (`REPLICAOF NO ONE` to a *running* replica) advances the epoch **without** restarting the process, so the marker stays behind; a later kill-9 then reads a greater epoch, treats a pre-death instruction as fresh, and returns as an **empty master** the operator believes is healthy — whereupon the replicas holding the only copy are repointed onto it. **Measured: 352 of 1145 acknowledged writes destroyed** (corruptions 0, write availability 95.50%), fixed to 0. The generalizable lesson: **the epoch is an *ordering* device that was answering an *identity* question** ("was this instruction issued for my current incarnation?"), and it failed exactly where ordering and identity diverge — protecting the *bootstrap* master (whose marker equals its master epoch) but no *promoted* one. The old e2e coverage matched the blind spot exactly, killing only bootstrap masters, which is why it survived 16/16. No reachability check before starting as replica (ADR-002's deadlock constraint).
...
- **Verification status**: unit/envtest green; **e2e-verified 16/16 on a real 3-node cluster (scm-s2, 2026-08-01)** — functional, graceful/crash failover (UID+RunID+label-agreement asserted), event path 4.96s (<15s bar), polling-only tier, the **hybrid double-failover graduation scenario** (promotion chain redis-0→1→2, the LR-007/LR-008/LR-024 class does not reproduce), kill-9 epoch-yield park (chaos: corruptions 0, write availability 96.5%), all three deadlock tiers (multi-holder same-lineage promotes with **no refuse** — the deliberate contrast to Rule L), rolling update with data intact. **Durability verified over 10 consecutive passes of the six-cell matrix (60 chaos runs, t3e, 2026-08-20): 0 MISSING and 0 corruptions in every block, all 10 passes green.** The headline is the kill-9 column, where the two modes' distributions **do not overlap**: failover 85.13-95.73% write availability against sentinel 43.32-73.89% — sentinel's best run is 11pp below failover's worst, with zero data loss on both sides. That is not a defect in either mode: sentinel's kill-9 guard suppresses Redis on the restarted master and waits for Sentinel to reach SDOWN and elect, while failover mode declares death from kubelet readiness and promotes in ~2s. It is the strongest single graduation-gate result, from the disruption shape that had **no coverage at all** before LR-038. `minReplicasToWrite: 1` costs nothing at the median (18.5 refused writes vs 16-19 with it off) with a ~20% tail costing ~45 more. Two cells are **bimodal, not noisy** (failover force-delete 13-20 vs 63-64; sentinel kill-9 ~314 vs ~600+) — a discrete ordering condition, undiagnosed, so never read a single sample of either. Earlier cross-cluster figures **(t3e single-node + s1 multi-node, 2026-08-19):** ten cells (both modes x three disruption shapes for failover, two for sentinel) lose **zero** acknowledged writes, and `failover >= sentinel` on both comparable variants in both environments. The kill-9 start-gate fix is confirmed off its discovery cluster (s1 reproduced the bug identically on the pre-fix build). Caution for readers: **kill-9 availability is noisy** (91.24/91.92/93.14/96.50 across single runs) — do not read a few points from one run; and the near-identical numbers across a loopback and a real-network cluster say this tier's availability cost is dominated by **operator reaction time**, not replication latency. Earlier single-cluster figures: **(t3e, 2026-08-18):** the full four-cell matrix of both modes' `rapid double failover` tiers now loses **zero** acknowledged writes — failover graceful 96.07% writes / 98.33% reads, failover crash 98.50% / 100.00%, sentinel graceful 95.41% / 98.25%, sentinel crash 97.58% / 99.50%. **Failover mode is at or better than sentinel on all four numbers**, which settles the gate's availability-vs-Sentinel question in its favour — and only because durability was measured first: the mode previously looked *better* than sentinel on write availability precisely while it was the only one of the two silently losing data (202 of 1171). Two earlier readings were wrong and are recorded as such: the residual crash loss was **not** an "inherent async tail" (sentinel-crash loses 0 under an identical cascade), and **not** the tier's `100m` CPU-limit asymmetry either (removing it left the loss at 20). It disappeared with the pod-local preStop fence. **Still unexplained:** why the *force-delete* path improved at all, since `--grace-period=0 --force` should skip preStop entirely — leading hypothesis is that the kubelet still runs its normal termination path because it never saw a zero-grace deletion object; the clean control is the kill-9 tier, where no hook can run. The ADR-011 **graduation gate remainder**: chaos/soak run + managed-cloud dogfooding evidence, then the drop/coexist/replace-sentinel decision. **Still unmeasured: the two sentinel-mode cells of the same tier**, so the cross-mode durability comparison is half-done.
