...
- **Verification status**: unit/envtest green; **e2e-verified 16/16 on a real 3-node cluster (scm-s2, 2026-08-01)** — functional, graceful/crash failover (UID+RunID+label-agreement asserted), event path 4.96s (<15s bar), polling-only tier, the **hybrid double-failover graduation scenario** (promotion chain redis-0→1→2, the LR-007/LR-008/LR-024 class does not reproduce), kill-9 epoch-yield park (chaos: corruptions 0, write availability 96.5%), all three deadlock tiers (multi-holder same-lineage promotes with **no refuse** — the deliberate contrast to Rule L), rolling update with data intact. **Durability re-verified after LR-038 (t3e, 2026-08-17):** `rapid double failover` — graceful loses **0** acknowledged writes (was 202, replicated across two runs); crash loses **19-20 of ~1194, cause open**. The four-cell matrix (both modes, one harness) reads: failover graceful 0 lost / 81.98% writes, failover crash 20 lost / 99.50%, sentinel graceful 0 lost / 94.75%, sentinel crash **0** lost / 97.50%. So (a) the graceful path reaches durability parity, (b) the fence costs ~2x sentinel's availability dip — ~216 failed writes ≈ the full remaining preStop window each failover, because the client's pinned connection is made to *fail* rather than moved off the dying pod (candidate: `CLIENT KILL` after the demote, untested), and (c) **failover-crash's residual loss is not the "inherent async tail" it was first called** — sentinel-crash loses 0 under an identical cascade. Leading suspect is a test-parity defect, not the mode: `failoverCR` pins a `100m` CPU *limit* while the sentinel chaos CR takes the product defaults, which per pillar 3.3 have **no CPU limit**; a throttled replica lags, and ~20 keys is ~2s of lag. No mode-level conclusion until that is equalised. The ADR-011 **graduation gate remainder**: chaos/soak run + managed-cloud dogfooding evidence, then the drop/coexist/replace-sentinel decision. **Still unmeasured: the two sentinel-mode cells of the same tier**, so the cross-mode durability comparison is half-done.
