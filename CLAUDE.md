...
### 3.14 Metadata Ownership & Inheritance
- **Decision**: Labels/annotations on the `LittleRed` CR are inherited by **every** owned resource (STS, Services, ConfigMap, PDBs, ServiceMonitor, pod templates); the operator's own keys always win and are never inherited; tool bookkeeping (`kubectl.kubernetes.io/`, `argocd.argoproj.io/`, `meta.helm.sh/`, `helm.sh/`, `*.toolkit.fluxcd.io/`) is not propagated. `spec.appName` supplies the `app.kubernetes.io/name` value (default `littlered`) and is **immutable**. (See ADR-015, issue #96.)
- **Why the naive "user labels win" cannot ship**: five keys are *structural* — `app.kubernetes.io/name`, `/instance`, `/component`, `redis.…/shard`, `/role` — they **are** the Service and StatefulSet selectors. `StatefulSet.spec.selector` is immutable, and under EmptyDir (pillar 3.1) the only way around an immutable selector — delete and recreate — **discards the data**. So a label typo would become data loss. Structural keys are therefore operator property: dropped from inheritance, dropped from `spec.podTemplate.labels`, and rejected up front by a CRD CEL rule so the error lands on the CR field instead of on a failing StatefulSet apply. `spec.appName`'s immutability is the same constraint made honest.
- **Implementation**: pure helpers in `internal/controller/metadata.go` (`inheritedLabels`, `objectLabels`, `podTemplateLabels`, `appNameFor`) — every builder goes through `commonLabels`/`inheritedAnnotations` (objects) or `podTemplateLabels`/`podTemplateAnnotations` (pods). **Never hardcode the `littlered` literal into a selector**; always call `appNameFor(lr)`. Precedence: inherited → operator-owned → `podTemplate.*` → per-object knobs (`service.*`, `metrics.serviceMonitor.labels`).
- **Consequence worth remembering**: pod labels live in the pod template, so editing CR metadata **rolls the pods** (sentinel failover; serialized per-shard roll in cluster mode; standalone restart = data discarded). Object-level metadata updates in place.
